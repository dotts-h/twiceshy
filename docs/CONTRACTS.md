...
| record schema + identity | on-disk record format (`schema_version: 1`, versioned JSON Schema in `schema/`; additive `provenance.source_license`/`source_url`/`security_flags`/`disputes` and the `guard.repros` test-set) and the `exp-NNNN` id `NextID` allocates and the path embeds — the contract between writer (`ingest`/`record`) and reader (`index`) | stable | [SCHEMA.md](SCHEMA.md), `schema/` |
