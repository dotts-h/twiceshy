...
| `ingest.Source` | `Name() string` + `Drafts(ctx) ([]Draft, error)`; a license-clean importer adapter emitting quarantined-record drafts; impls `deprecationSource` (go/py) + `osvSource` | stable | `internal/ingest` ([ADR-0003](adr/ADR-0003-corpus-bootstrap-source-scope.md)) |
...
| record schema + identity | on-disk record format (`schema_version: 1`, versioned JSON Schema in `schema/`; additive `provenance.source_license`/`source_url`/`security_flags`) and the `exp-NNNN` id `NextID` allocates and the path embeds — the contract between writer (`ingest`/`record`) and reader (`index`) | stable | [SCHEMA.md](SCHEMA.md), `schema/` |
