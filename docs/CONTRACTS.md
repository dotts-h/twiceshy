...
| `index.Index` | methods `Open`/`Close`/`Rebuild`/`Search`/`Retrieve`/`Assess`/`Get`/`NextID`; values `Query`/`Hit`/`Stored`/`Novelty`/`Assessment`; precedence **fingerprint-exact → lexical**, **`MaxK` hard cap**, relevance floor as index policy on every injection path — `Retrieve`/`Assess` apply it, `Search` is the floor-free mechanism ([ADR-0004](adr/ADR-0004-relevance-floor-is-index-policy.md), [ADR-0007](adr/ADR-0007-floor-on-the-read-path.md)); scores positive/higher-is-better, embedding-free hot path | stable | `internal/index` ([ADR-0001 §3–4](adr/ADR-0001-architecture.md)) |
