...
> **Authoritative layout: [CODEBASE_MAP.md](CODEBASE_MAP.md).** The bullets below are
> a curated subset that carries the *why* (the layering + the invariant-bearing
> packages); the full, current package list (21 `internal/*` packages) and the entry
> points live in CODEBASE_MAP.md so this narrative can't silently drift from the tree.
> The per-symbol index is [CODEMAP.md](CODEMAP.md) (generated by `scripts/codemap.sh`).

- **`cmd/twiceshy/`** *(edge — entry)* — the binary. Thin `main` → `run(ctx,
  args, out, getenv)` delegating to `internal/`; the subcommands (`index`, `serve`,
  `ingest`, `promote`/`adapt`, `intake-*`, `doctor`, `eval`, …) are listed in
  CODEBASE_MAP.md.
...
- **`internal/server/`** *(edge — MCP/HTTP)* — the MCP tools (`search_experience`,
  `get_experience`, `record_experience`, `report_outcome`, `report_issue`,
  `confirm_helpful`) + the push hook over streamable HTTP, bearer-gated, behind a
  middleware chain (auth → rate-limit → timeout → max-bytes). Translates tool args to
  core calls and back; holds no domain logic.
- *(The autonomous-validation subsystem — `judge`, `judgeeval`, `repro`, `promote`,
  `drafter`, `retro`, `spool`, `telemetry`, `selfaudit`, `guard`, `eval`, `notify`,
  `lock` — is the bulk of the tree; see CODEBASE_MAP.md §"VALIDATION loop".)*
