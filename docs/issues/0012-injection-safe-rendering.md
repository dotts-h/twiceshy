...
## Scope
- [x] **Data framing:** render returned/injected records inside an explicit,
...
- [x] **Escaping:** neutralize delimiter-breakout (fence/backtick/control chars)
      in record fields before rendering.
- [x] **Caps:** per-field and per-card length caps at render time (complements the
      k≤3 retrieval cap).
- [x] The push half lands with #0002 (the hook + trap-card renderer); the pull
      half (`get_experience`/`search_experience` response shape + `status` label)
      can land independently now.

## Acceptance
- [x] A record whose body contains a fenced block / "ignore previous instructions"
      payload is rendered as inert data (cannot break the envelope); guarding test.
- [x] Only `validated` records are eligible for the push renderer.
