...
| [0098](0098-activate-the-measurement-chain-in-production-enable-serve-telemetry-cross-host-decision-log-access-so-the-0069-helpfulness-join-runs-live.md) | Activate the measurement chain in production — cross-host decision-log sync so the #0069 helpfulness join runs live on real traffic | open | medium | 0064 | |
...
| [0140](0140-live-prospector-run-17-record-re-run-with-positive-controls-model-hard-set-or-honest-null-deferred-from-0119.md) | Live prospector run — 17-record re-run with positive controls; model-hard set or honest null (deferred from 0119) | open | high | 0112 | |
...
| [0142](0142-prospect-run-aborts-on-npm-e404-for-a-record-derived-dep-deterministic-dep-not-found-must-be-a-counted-per-case-skip-deps-not-a-run-level-error.md) | Prospect run aborts on npm E404 for a record-derived dep — deterministic dep-not-found must be a counted per-case skip (deps), not a run-level error | open | high | 0112 | |
