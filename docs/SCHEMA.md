...
| `source_license` | string | no | SPDX id (e.g. `CC-BY-4.0`, `MIT`), `none (facts only)` (a distilled license-clean fact, [ADR-0003](adr/ADR-0003-corpus-bootstrap-source-scope.md) §4), `none (project-authored)` (explicit evidence that the record's expression is project-authored; no `source_url`), or `none (authored, internal-only)` (a fact re-derived from a public-awareness *topic*, [ADR-0011](adr/ADR-0011-corpus-growth-and-validation-engine.md) §5 — born with no `source_url`, kept out of commercial packs pending a real legal review). The field remains optional for legacy compatibility, but an empty value is not commercial-pack rights evidence. Copied licensed material also needs `source_url` so the pack can emit its source/license notice |
| `source_url` | string | no | `http(s)` URL the imported fact was distilled from; recorded alongside `source_license` |
| `source_attribution` | object | no | Explicit third-party notice evidence: optional `{creator, title, license_url, changes, copyright_notice, notice, license_text}` strings. Partial evidence is schema-valid during remediation, but the commercial pack classifier fails closed unless the fields required by the declared `source_license` are complete. Values are never inferred |
| `rights_review` | object | no | Immutable human review attestation `{reviewer, reviewed_at, source_sha256, evidence_sha256, policy}`. Commercial eligibility requires the approved policy version, non-placeholder reviewer identity, RFC3339 time, source digest, and an evidence digest matching the record's exact rights fields. Never auto-populated; not a claim of counsel approval |
