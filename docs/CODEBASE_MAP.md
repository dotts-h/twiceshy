...
| `internal/index/` | Derived, always-rebuildable SQLite/FTS5 index over the corpus and the **embedding-free hot path**: fingerprint-exact → BM25 lexical, hard cap k≤3, relevance floor. `Open`/`Rebuild`/`Search`/`Get`/`Assess`; the push gate (`RetrievePush`/`RetrievePushTraced`); dense/RRF (`dense.go`) lives here but stays *off* the hot path. The only stateful package. The db file also carries the durable tenant registry (tokens, organization/workspace assignments, entitlements, and per-tenant counters) — non-derived, survives Rebuild, never delete on a tenant-serving deployment (ADR-0034/0035). | §1, §3–4, [9](adr/ADR-0009-dense-retrieval-is-pure-go-cosine.md) |
| `internal/entitlement/` | Closed plan vocabulary (`community`, `pro`, `team`, `enterprise`) and the quota policies derived from it. No billing/provider state; feature-gated by the admin CLI (ADR-0035). | [35](adr/ADR-0035-feature-gated-team-plan-foundation.md) |
...
| `internal/measurement/` | Privacy-safe design-partner pilot reporting: joins salted decision/session telemetry to strict outcomes, compares baseline/treatment windows, and emits confidence-aware aggregate/team/record metrics. | [Pilot protocol](PILOT_PROTOCOL.md) |
