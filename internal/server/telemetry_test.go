...
// The push handler records a per-query gate decision (#0067): the served ids +
// scores + the discriminative gate tokens for a hit, an empty decision for an
// off-topic query — and the raw query is hashed, never persisted.
func TestTelemetryRecordsPushGateDecision(t *testing.T) {
	// BM25 + the discriminative-df gate are corpus-relative (exp-0098), so this
	// exercises the gate against the real corpus, the way push runs in production.
