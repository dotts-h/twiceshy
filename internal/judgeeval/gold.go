...
// advisoryGoldYAML is the generated advisory gold set (#0074): the Sonnet advisory
// verdicts as advisory-class cases. Kept in a separate embed (regenerated by
// `gold-add -advisory-audit`) so the hand-written prose gold.yaml stays readable.
//
//go:embed advisory-gold.yaml
var advisoryGoldYAML []byte
...
func LoadGold() ([]Case, error) {
	...
	// Merge the generated advisory gold set (#0074); the validation below runs over the
	// combined set (duplicate id across both files, consistency, advisory routing).
	var af goldFile
	if err := yaml.Unmarshal(advisoryGoldYAML, &af); err != nil {
		return nil, fmt.Errorf("judgeeval: parsing advisory-gold.yaml: %w", err)
	}
	gf.Cases = append(gf.Cases, af.Cases...)
