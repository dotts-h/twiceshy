...
func TestPromoteCorpus_EmergencyStopHalts(t *testing.T) {
	...
	st, _, err := promoteCorpus(context.Background(), ".", recs, fp, persist, guard.Guardrails{Paused: true}, nil, &buf)
	if err != nil {
...
func TestPromoteCorpus_BudgetCapStops(t *testing.T) {
	...
	st, _, err := promoteCorpus(context.Background(), ".", recs, fp, persist, guard.Guardrails{MaxRuns: 1}, nil, &buf)
	if err != nil {
		t.Fatalf("promoteCorpus: %v", err)
	}
...
func TestAdaptCorpus_EmergencyStopHalts(t *testing.T) {
	...
	st, _, err := adaptCorpus(context.Background(), ".", recs, runner, adapter, persist, guard.Guardrails{Paused: true}, nil, &buf)
	if err != nil {
...
func TestAdaptCorpus_BudgetCapStops(t *testing.T) {
	...
	st, _, err := adaptCorpus(context.Background(), ".", recs, runner, adapter, persist, guard.Guardrails{MaxRuns: 1}, nil, &buf)
	if err != nil {
		t.Fatalf("adaptCorpus: %v", err)
	}
	if st.demoted != 1 {
